...
namespace Sitecore.Extranet.Core.Utility
{
    public class ExtranetSecurity
    {
        ...
        {
            return true;
            //TODO: make hasextranetuserprefix optional
            //PropExists(Constants.ExtranetAttributes.UserPrefix);
        }
        ...
        {
            //if you're logged in and you have the extranet role
            string qualRole = string.Format(@"{0}\{1}", Sitecore.Context.Domain, ExtranetRole());
            return (Sitecore.Context.IsLoggedIn && Sitecore.Context.User.IsInRole(qualRole));
        }
